...
namespace H5.Translator
{
    public abstract partial class ConversionBlock
    {
        ...
        private static void NarrowingNumericOrEnumerationConversion(ConversionBlock block, Expression expression, IType targetType, bool fromFloatingPoint, bool isChecked, bool isNullable, bool isExplicit = true)
        {
            ...
            if (isChecked)
            {
                block.Write(JS.Types.H5_INT + ".check(");

                if (fromFloatingPoint)
                {
                    block.Write(JS.Types.H5_INT + ".trunc");
                    block.WriteOpenParentheses();
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        block.Write(JS.Types.H5_INT + ".sxb(");
                        if (!skipInnerWrap)
                        {
                            block.WriteOpenParentheses();
                            block.AfterOutput += ")";
                    ...
                    {
                        block.Write(JS.Types.H5_INT + ".sxs(");
                        if (!skipInnerWrap)
                        {
                            block.WriteOpenParentheses();
                            block.AfterOutput += ")";
                    ...
                    {
                        block.Write(JS.Types.H5_INT + ".clip64(");
                        block.AfterOutput += ")";
                    }
                    else if (targetType.IsKnownType(KnownTypeCode.UInt64))
                    {
                        block.Write(JS.Types.H5_INT + ".clipu64(");
                        block.AfterOutput += ")";
                    }
                    else
                    {
