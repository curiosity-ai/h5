...
namespace HighFive.Translator
{
    public abstract partial class ConversionBlock
    {
        ...
        private static void NarrowingNumericOrEnumerationConversion(ConversionBlock block, Expression expression, IType targetType, bool fromFloatingPoint, bool isChecked, bool isNullable, bool isExplicit = true)
        {
            ...
            if (isChecked)
            {
                block.Write(JS.Types.HIGHFIVE_INT + ".check(");

                if (fromFloatingPoint)
                {
                    block.Write(JS.Types.HIGHFIVE_INT + ".trunc");
                    block.WriteOpenParentheses();
                }
            ...
            else
            {
                if (isNullable || fromFloatingPoint)
                {
                    ...
                    block.Write(JS.Types.HIGHFIVE_INT + ".");
                    block.Write(action);
                    block.Write("(");
                    block.AfterOutput += ")";
                }
                else
                {
                    ...
                    {
                        block.Write(JS.Types.HIGHFIVE_INT + ".sxb(");
                        if (!skipInnerWrap)
                        {
                            block.WriteOpenParentheses();
                            block.AfterOutput += ")";
                    ...
                    {
                        block.Write(JS.Types.HIGHFIVE_INT + ".sxs(");
                        if (!skipInnerWrap)
                        {
                            block.WriteOpenParentheses();
                            block.AfterOutput += ")";
                    ...
                    {
                        block.Write(JS.Types.HIGHFIVE_INT + ".clip64(");
                        block.AfterOutput += ")";
                    }
                    else if (targetType.IsKnownType(KnownTypeCode.UInt64))
                    {
                        block.Write(JS.Types.HIGHFIVE_INT + ".clipu64(");
                        block.AfterOutput += ")";
                    }
                    else
                    {
