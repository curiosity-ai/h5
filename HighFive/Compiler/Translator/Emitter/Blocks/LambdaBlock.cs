        ...
        protected virtual void EmitLambda(IEnumerable<ParameterDeclaration> parameters, AstNode body, AstNode context)
        {
            ...
            if (!noLiftingRule && analyzer.UsedVariables.Count == 0)
            {
                if (!this.Emitter.ForbidLifting)
                {
                    ...
                    this.Emitter.Output.Insert(savedPos, JS.Vars.D_ + "." + HighFiveTypes.ToJsName(this.Emitter.TypeInfo.Type, this.Emitter, true) + "." + name);
                }
            ...
            if (thisCaptured)
            {
                this.Emitter.Output.Insert(savedPos, JS.Funcs.HIGHFIVE_BIND + (hasCapturedVariables ? "($me, " : "(this, "));
